...
//	http://localhost[:8080]/{your-app-root}/swagger-ui/index.html
	...
	private String title = "Tourmate REST API " + version;

	private ApiInfo apiInfo() {
		String descript = "SSAFY final project API Reference for Developers<br>";
...
//				.termsOfServiceUrl("https://edu.ssafy.com")
	...
	public Docket userApi() {
		return getDocket("5. 회원", Predicates.or(PathSelectors.regex("/user.*")));
	}

	@Bean
	public Docket noticeApi() {
		return getDocket("4. 공지사항", Predicates.or(PathSelectors.regex("/notice.*")));
	}

	@Bean
	public Docket tourApi() {
		return getDocket("2. 여행지", Predicates.or(PathSelectors.regex("/tour.*")));

	}
	
	@Bean
	public Docket planApi() {
		return getDocket("3. 여행 계획", Predicates.or(PathSelectors.regex("/plan.*")));
...
//		responseMessages.add(new ResponseMessageBuilder().code(404).message("페이지를 찾을 수 없습니다 !!!").build());
		return new Docket(DocumentationType.SWAGGER_2).groupName(groupName).apiInfo(apiInfo()).select()
				.apis(RequestHandlerSelectors.basePackage("com.ssafy.tourmate.controller")).paths(predicate)
